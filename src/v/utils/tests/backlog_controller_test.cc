...
struct simple_backlog_sampler : backlog_controller::sampler {
...
struct backlog_controller_fixture {
    ...
    backlog_controller_fixture()
      : iopc(ss::engine().register_one_priority_class("io_control_gr", 100)) {
         ...
         * max shares = 800
         */
        auto cfg = backlog_controller_config(
          -1.0, 0.4, 0.2, 20, 100, 100ms, sg, iopc, 10, 800);

        ctrl = std::make_unique<backlog_controller>(
          std::make_unique<simple_backlog_sampler>(backlog), ctrl_logger, cfg);
    }

    ss::io_priority_class iopc;
    ss::scheduling_group sg;
    std::unique_ptr<backlog_controller> ctrl;
    int64_t backlog{0};
...
FIXTURE_TEST(test_feedback_loop, backlog_controller_fixture) {
     ...
     *
     * Expected amount of shares => 68;
     */
    tests::cooperative_spin_wait_with_timeout(500ms, [this] {
        return get_current_cpu_shares() == 68;
    }).get();
    /**
