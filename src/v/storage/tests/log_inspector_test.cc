...
TEST(compacted_reads, compare_read_methods) {
    ...
    for (int pod = 0; pod < 6; ++pod) {
        ...
        auto dir = fmt::format(
          "/home/mmaslanka/dev/support/wunderkind-11-04-02/pod-{}", pod);
        ...
        for (int part = 0; part < 16; ++part) {
            ...
            if (revision != model::revision_id(0)) {
                ...
                log->start(std::nullopt).get();
                {
                    ...
                    auto g_state
                      = rdr.consume(std::move(consumer), model::no_timeout)
                          .get();
                    fmt::print(
        ...
        for (auto& [partition, group_state] : partition_groups) {
            for (auto& [group_id, group_stm] : group_state.groups) {
                fmt::print(
                  ...
                  partition);
                // fmt::print("group_stm: {}\n", group_stm);
                groups_stms[group_id][pod] = std::make_unique<kafka::group_stm>(
                  std::move(group_stm));
            }
        }
    }
    for (auto& [group_id, pod_groups] : groups_stms) {
        ...
        for (auto& [partition, pod_offsets] : replica_offsets) {
            absl::node_hash_set<std::pair<model::offset, int64_t>> offsets;
            for (auto& [pod, omd] : pod_offsets) {
                fmt::print(
                  ...
                  omd.metadata);
                offsets.emplace(
                  omd.metadata.offset, omd.metadata.commit_timestamp.value());
            }
            if (offsets.size() > 1) {
                std::vector<int64_t> time_diffs;
                bool large = false;
                for (auto& o : offsets) {
                    auto diff = offsets.begin()->second - o.second;
                ...
                fmt::print(
                  ...
                  fmt::join(
                    std::views::transform(
                      offsets, &std::pair<model::offset, int64_t>::first),
                    ", "),
                  fmt::join(time_diffs, ", "),
                  large ? "LARGE" : "");
            }
...
//         for (size_t i = 0; i < random_generators::get_int<size_t>(1,
//         per_step);
//              ++i) {
//             auto key = fmt::format(
...
//                 builder.add_raw_kv(
//                   serde::to_iobuf(r.first),
//                   serde::to_iobuf(r.second.value()));
//             } else {
//                 // tombstone
