...
public:
    ...
    public:
        static constexpr size_t range_size = 128_KiB;
        class lock_guard {
        public:
        ...
        explicit range(batch_cache_index& index)
          : _index(index) {
            _arena.reserve_memory(range_size);
        }
    ...
    batch_cache(batch_cache&& o) noexcept
      ...
      , _reclaim_size(_reclaim_opts.min_size)
      , _background_reclaimer(std::move(o._background_reclaimer)) {
        o._size_bytes = 0;
...
private:
    ...
    private:
        ...
        bool _stopped = false;
        ss::condition_variable _change;
        batch_cache& _cache;
        size_t _min_free_memory;
        ss::scheduling_group _sg;
        ss::future<> _reclaim_loop = ss::now();
    };
