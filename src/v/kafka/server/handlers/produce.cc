            ...
            [reader = std::move(reader),
             ...
             acks = octx.request.data.acks](
              cluster::partition_manager& mgr) mutable {
                ...
                if (!partition) {
                    return std::move(dispatch).dispatch_set_value().then(
                      [pid = ntp.tp.partition] {
                ...
                return stages.dispatched
                  .then([dispatch = std::move(dispatch)]() mutable {
                      // dispatch back to where it came from
                      return std::move(dispatch).dispatch_set_value();
                  })
