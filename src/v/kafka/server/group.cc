...
group::join_group_unknown_member(join_group_request&& r) {
    if (in_state(group_state::dead)) {
        ...
        return join_group_stages(_make_join_error(
    ...
    if (r.version >= api_version(4) && !r.data.group_instance_id) {
        ...
        return join_group_stages(
          _make_join_error(new_member_id, error_code::member_id_required));
    } else {
...
group::join_group_known_member(join_group_request&& r) {
    if (in_state(group_state::dead)) {
        ...
        vlog(
          ...
          r.data.member_id);
        return join_group_stages(
          _make_join_error(r.data.member_id, error_code::unknown_member_id));
    }
...
group::join_group_stages group::add_member_and_rebalance(
  kafka::member_id member_id, join_group_request&& r) {
    ...
    return join_group_stages(ss::now(), std::move(response));
    ;
}
  ...
  member_ptr member, sync_group_request&& r) {
    ...
    if (!is_leader(r.data.member_id)) {
        vlog(
          ...
          member->id());
        return sync_group_stages(ss::now(), std::move(response));
    }
    ...
    return sync_group_stages(ss::now(), std::move(f));
}
