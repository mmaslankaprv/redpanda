...
public:
    ...
    auto execute_query(
      ...
      std::optional<matcher<manifest>> manifest_matcher = std::nullopt) {
        query_executor executor(io, table);
    ...
    auto execute_for_all_results(
      const table_metadata& table,
      std::optional<std::function<bool(const snapshot&)>> snapshot_matcher
      = std::nullopt,
      std::optional<std::function<bool(const manifest_file&)>>
        manifest_file_matcher
      = std::nullopt,
      std::optional<std::function<bool(const manifest&)>> manifest_matcher
      = std::nullopt) {
        query_executor executor(io, table);
...
TEST_F(MetadataQueryTest, TestQueryManifestFile) {
    auto tx = generate_metadata().get();
    // manifest file query, only those manifest files which partition key lower
    // bound is greater or equal than 3
    auto matcher = [](const manifest_file& file) {
        return *(reinterpret_cast<const int*>(
                 file.partitions[0].lower_bound->cbegin()))
               >= 3;
    };
...
TEST_F(MetadataQueryTest, TestQueryManifests) {
    ...
    metadata_query<result_type::snapshot> snap_query{.manifest_matcher = matcher};
    metadata_query<result_type::manifest_file> mf_query{.manifest_matcher = matcher};
    metadata_query<result_type::manifest> m_query{.manifest_matcher = matcher};
...
AssertionResult
check_query_result_is_empty(const query_executor& executor, const TupleT& tp) {
...
TEST_F(MetadataQueryTest, TestQueryMatchingNone) {
    ...
    auto queries = std::make_tuple(
      metadata_query<result_type::snapshot>{.snapshot_matcher = match_none<snapshot>},
      metadata_query<result_type::snapshot>{
        .manifest_file_matcher = match_none<manifest_file>},
      metadata_query<result_type::snapshot>{.manifest_matcher = match_none<manifest>},
      ...
      metadata_query<result_type::manifest>{.snapshot_matcher = match_none<snapshot>},
      metadata_query<result_type::manifest>{
        .manifest_file_matcher = match_none<manifest_file>},
      metadata_query<result_type::manifest>{.manifest_matcher = match_none<manifest>});

    query_executor executor(io, tx.table());
