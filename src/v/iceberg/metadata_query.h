...
using matcher = ss::noncopyable_function<bool(const ElemT&)>;
/**
...
public:
    ...
    explicit query_executor(manifest_io& io, const table_metadata& table)
      : io_(io)
      , table_(table) {}
...
private:
    template<result_type ResultT>
    ss::future<
      checked<typename internal::results_container<ResultT>::type, errc>>
    do_execute_query(const metadata_query<ResultT>& query) const;
