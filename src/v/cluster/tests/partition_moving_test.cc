  ...
  const std::vector<cluster::topic_result>& results) {
    tests::cooperative_spin_wait_with_timeout(
      ...
      .handle_exception([](const std::exception_ptr& e) {
          BOOST_FAIL("Timeout waiting for topic metadata update");
      })
      .get0();
}
...
struct validation_opts {
    bool should_wait_for_metadata{true};
    bool should_wait_for_replicas{true};
    bool should_wait_for_recovery{true};
};
...
public:
        ...
        wait_for_all_replicas(
          ...
          [this, ntp = std::move(ntp), &replicas](
            const model::broker_shard& bs) mutable {
              return print_configuration(ntp).then([this, bs, &replicas, ntp] {
          ...
          .finally([ntp, this] { return print_configuration(ntp); })
          .get0();
    }
    ...
    foreign_batches_t execute_and_validate_history_updates(
      ...
      validation_opts vo = validation_opts()) {
        ...
        for (const auto& r : history) {
            ...
            get_leader_controller()
              ->get_topics_frontend()
              .local()
              .move_partition_replicas(ntp, r, model::no_timeout)
              .get0();

            // check topics table
            if (vo.should_wait_for_metadata) {
                wait_for_metadata_update(ntp, r);
            }
            // check if replicas are there
            if (vo.should_wait_for_replicas) {
                wait_for_replica_set_partitions(20s, ntp, r);
            }
            if (vo.should_wait_for_recovery) {
 ...
 * - random history update without waiting for intermediate states
...
FIXTURE_TEST(multiple_changes_test_no_wait, partition_assignment_test_fixture) {
    model::ntp ntp(test_ns, model::topic("t_1"), model::partition_id(0));
    create_topic(test_topics_configuration(
    ...
    validation_opts opts{
      .should_wait_for_metadata = false,
      .should_wait_for_replicas = false,
      .should_wait_for_recovery = false};

    auto reference_batches = execute_and_validate_history_updates(
      ntp, history, opts);
    ...
    validate_replicas_recovery(10s, ntp, history.back(), reference_batches);
}

// Test convering the history of updates that may lead to disjoint  quorums if
// hanled incorrectly
FIXTURE_TEST(corner_cases, partition_assignment_test_fixture) {
    ...
    // here we have to wait for a long time as the cluster must reach all
    // intermediate states before reaching this point
    wait_for_replica_set_partitions(30s, ntp, history.back());
    validate_replicas_recovery(10s, ntp, history.back(), reference_batches);
}
...
FIXTURE_TEST(
  multiple_changes_test_multi_topics, partition_assignment_test_fixture) {
    ...
    ss::when_all(all.begin(), all.end()).get0();
}
...
FIXTURE_TEST(disjoint_qorums, partition_assignment_test_fixture) {
    ...
    validate_replicas_recovery(2s, ntp, history.back(), reference_batches);
}
...
FIXTURE_TEST(failure_recovery, partition_assignment_test_fixture) {
    model::ntp ntp(test_ns, model::topic("t_1"), model::partition_id(0));
    create_topic(test_topics_configuration(
      model::topic_namespace(ntp.ns, ntp.tp.topic), 3));
    auto current = get_replicas(0, ntp);
    ...
    auto reference_batches = execute_and_validate_history_updates(
      ntp, history, opts);
    size_t last_success = random_generators::get_int(history.size() - 3);
    // wait for certain point in history (not the last one)
    wait_for_replica_set_partitions(60s, ntp, history[last_success]);
    BOOST_TEST_MESSAGE(
      fmt::format("Restarting after {}/{}", last_success, history_size));
    // restart cluster

    restart_cluster();
    // recovery should apply all the changes
    wait_for_metadata_update(ntp, history.back());
    wait_for_replica_set_partitions(120s, ntp, history.back());
    validate_replicas_recovery(2s, ntp, history.back(), reference_batches);
}
