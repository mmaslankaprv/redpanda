...
ss::future<bool>
controller_backend::is_applicable(model::ntp, const topic_table::delta& d) {
    model::revision_id rev(d.offset());
    // only in progress updates are applicable
    if (d.type != topic_table::delta::op_type::update) {
...
std::vector<topic_table::delta> calculate_bootstrap_deltas(
  model::node_id self, std::vector<topic_table::delta>&& deltas) {
    ...
    while (it != deltas.rend()) {
        ...
        // replicas, just stop
        if (
          it->type == topic_table::delta::op_type::update_finished
        ...
        if (auto next = std::next(it); next != deltas.rend()) {
            if (
              next->type == topic_table::delta::op_type::update_finished
...
void controller_backend::housekeeping() {
    if (!_topic_deltas.empty() && _topics_sem.available_units() > 0) {
        (void)ss::with_gate(_gate, [this] {
            ...
            vlog(clusterlog.error, "House keeping error - {}", e);
        });
        ;
    }
}
...
bool is_configuration_up_to_date(
  ...
  model::revision_id change_revision) {
    ...
    /**
     * if current configuration includes itself we have to wait to become a
     * voter, if we are going to remove the partition it is ok for the node to
     * be a learner
     */
    if (!includes_self) {
...
ss::future<std::error_code> controller_backend::process_partition_update(
  model::ntp ntp, const partition_assignment& current, model::revision_id rev) {
    ...
    if (!has_local_replicas(_self, current.replicas)) {
        ...
        /**
         * if current node wasn't able to update the partition configuration,
         * wait for other node to do it, retry during next reconciliation loop.
         */
    ...
    co_return co_await create_partition(ntp, current.group, rev, {})
      .then([](std::error_code) {
          // wait for recovery, we will mark partition as updated in next
          // controller backend reconciliation loop pass
          return std::error_code(errc::waiting_for_recovery);
      });
}
...
ss::future<std::error_code>
controller_backend::delete_partition(model::ntp ntp, model::revision_id) {
