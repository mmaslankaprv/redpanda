...
ss::future<> replicate_batcher::do_flush(
  ...
  absl::flat_hash_map<vnode, follower_req_seq> seqs) {
    ...
    try {
        auto holder = _bg.hold();
        auto leader_result = co_await stm->append_to_leader(std::move(u));
        ...
        propagate_result(
          leader_result,
          notifications,
          [has_error = leader_result.has_error()](const item_ptr& item) {
              return has_error
                     || item->consistency_lvl == consistency_level::leader_ack
