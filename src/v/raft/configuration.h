...
public:
    ...
    vnode(model::node_id nid, model::revision_id rev)
      : _node_id(nid)
      , _revision(rev) {}

    bool operator==(const vnode& other) const {
        return _node_id == other._node_id && _revision == other._revision;
    }

    bool operator!=(const vnode& other) const { return !(*this == other); }
...
public:
    ...
    void set_revision(model::revision_id rev) { _revision = rev; }
    /**
