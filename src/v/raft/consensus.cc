...
void consensus::process_append_entries_reply(
  ...
  model::offset dirty_offset) {
    auto is_success = update_follower_index(
      expected_vnode, r, seq_id, dirty_offset);
    if (is_success) {
  ...
  std::vector<model::broker> nodes, std::optional<model::revision_id> rev) {
    ...
    return change_configuration(
      [nodes = std::move(nodes), rev](group_configuration current) mutable {
          ...
          auto revision = rev.value_or(current.revision_id()++);
          current.add(std::move(nodes), revision);
          current.set_revision(revision);
...
ss::future<std::error_code> consensus::replace_configuration(
  ...
  std::optional<model::revision_id> rev) {
    return change_configuration([new_brokers = std::move(new_brokers),
        ...
        auto revision = rev.value_or(current.revision_id()++);
        current.replace(std::move(new_brokers), revision);
        current.set_revision(revision);
        return result<group_configuration>(std::move(current));
    });
}
    ...
    return _op_lock.get_units().then(
      [this, ids = std::move(ids)](ss::semaphore_units<> u) mutable {
          ...
          auto all_present = std::all_of(
            std::cbegin(ids), std::cend(ids), [&cfg](model::node_id id) {
    ...
    auto f = ss::with_gate(_bg, [this, target_rni = *target_rni] {
        ...
        return f.then([this, target_rni] {
            ...
            return _client_protocol
              .timeout_now(
                target_rni, std::move(req), rpc::client_opts(timeout))
